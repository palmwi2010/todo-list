...
function render($container) {
    ...
    function createButton(btn) {
        ...
        const btnUpper = btnType[0].toUpperCase() + btnType.slice(1);
    
        const $img = createElement({"type": "img", "elemClass": "icon", "id": `${btnType}-icon`});
    ...
    // Sorting section
    const $sortDiv = createElement({"type": "div", "elemClass": "sort-div"});
    const $label = createElement({"type": "p", "elemClass": "sort-label", "elemText":"Sort by:"});
    $sortDiv.appendChild($label)
    const $datebnCreate =  createElement({"type": "button", "elemClass": "sort-btn", "id": "sortCreated", "elemText":"Created date"});
    $datebnCreate.classList.add("sort-active");
    const $datebnDue =  createElement({"type": "button", "elemClass": "sort-btn", "id": "sortDue", "elemText":"Due date"});
